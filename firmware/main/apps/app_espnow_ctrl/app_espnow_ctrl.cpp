...
void AppEspnowControl::onOpen()
{
    ...
    // Start espnow
    GetHAL().startEspNowStandalone(_wifi_channel);
...
void handle_received_data()
{
    ...
    if (!_received_data.empty()) {
        stackchan::features::SetEspNowReceiverId(_receiver_id);
        stackchan::features::HandleEspNowPacket(_received_data);
    }
