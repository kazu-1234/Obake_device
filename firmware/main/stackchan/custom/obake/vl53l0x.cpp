...
void VL53L0X::setI2c(Vl53I2cWriteFn write_fn, Vl53I2cWriteReadFn write_read_fn, void* user)
{
  write_fn_ = write_fn;
  write_read_fn_ = write_read_fn;
  i2c_user_ = user;
}
...
void VL53L0X::writeReg(uint8_t reg, uint8_t value)
{
  ...
  last_status = (write_fn_ && write_fn_(address, buf, 2, i2c_user_)) ? 0 : 1;
}

void VL53L0X::writeReg16Bit(uint8_t reg, uint16_t value)
{
  uint8_t buf[3] = {reg, (uint8_t)(value >> 8), (uint8_t)value};
  last_status = (write_fn_ && write_fn_(address, buf, 3, i2c_user_)) ? 0 : 1;
}

void VL53L0X::writeReg32Bit(uint8_t reg, uint32_t value)
{
  uint8_t buf[5] = {reg, (uint8_t)(value >> 24), (uint8_t)(value >> 16), (uint8_t)(value >> 8), (uint8_t)value};
  last_status = (write_fn_ && write_fn_(address, buf, 5, i2c_user_)) ? 0 : 1;
}

uint8_t VL53L0X::readReg(uint8_t reg)
{
  uint8_t value = 0;
  last_status = (write_read_fn_ && write_read_fn_(address, &reg, 1, &value, 1, i2c_user_)) ? 0 : 1;
  return value;
}
...
{
  uint8_t raw[2] = {0, 0};
  last_status = (write_read_fn_ && write_read_fn_(address, &reg, 1, raw, 2, i2c_user_)) ? 0 : 1;
...
uint32_t VL53L0X::readReg32Bit(uint8_t reg)
{
  ...
  last_status = (write_read_fn_ && write_read_fn_(address, &reg, 1, raw, 4, i2c_user_)) ? 0 : 1;
...
{
  uint8_t buf[64];
  if (count + 1 > sizeof(buf)) { last_status = 1; return; }
  buf[0] = reg;
  memcpy(buf + 1, src, count);
  last_status = (write_fn_ && write_fn_(address, buf, count + 1, i2c_user_)) ? 0 : 1;
}

void VL53L0X::readMulti(uint8_t reg, uint8_t * dst, uint8_t count)
{
  last_status = (write_read_fn_ && write_read_fn_(address, &reg, 1, dst, count, i2c_user_)) ? 0 : 1;
}
