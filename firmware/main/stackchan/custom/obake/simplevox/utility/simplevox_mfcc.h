...
namespace simplevox
{
    ...
    public:
        ...
        /** NVS 等向け: メモリバッファへ保存／から読込（ファイル形式と同じ） */
        static bool saveMemory(const MfccFeature& mfcc, uint8_t* dest, size_t dest_cap, size_t* out_len);
        static MfccFeature* loadMemory(const uint8_t* data, size_t len);
