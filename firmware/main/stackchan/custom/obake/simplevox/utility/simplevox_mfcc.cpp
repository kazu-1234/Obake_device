...
namespace simplevox
{
    ...
    bool MfccEngine::saveMemory(const MfccFeature& mfcc, uint8_t* dest, size_t dest_cap, size_t* out_len)
    {
        if (!dest || !out_len) {
            return false;
        }
        const int32_t size = mfcc.size();
        const int32_t coef_num = mfcc.dimension();
        const size_t need = 1 + sizeof(size) + sizeof(coef_num) + static_cast<size_t>(size) * coef_num * sizeof(int16_t);
        if (dest_cap < need) {
            return false;
        }
        size_t off = 0;
        dest[off++] = static_cast<uint8_t>(MfccTag::VERSION1);
        memcpy(dest + off, &size, sizeof(size));
        off += sizeof(size);
        memcpy(dest + off, &coef_num, sizeof(coef_num));
        off += sizeof(coef_num);
        memcpy(dest + off, mfcc.feature_, static_cast<size_t>(size) * coef_num * sizeof(int16_t));
        *out_len = need;
        return true;
    }

    MfccFeature* MfccEngine::loadMemory(const uint8_t* data, size_t len)
    {
        if (!data || len < 1 + sizeof(int32_t) * 2) {
            return nullptr;
        }
        size_t off = 0;
        const auto tag = static_cast<MfccTag>(data[off++]);
        (void)tag;
        int32_t size = 0;
        int32_t coef_num = 0;
        memcpy(&size, data + off, sizeof(size));
        off += sizeof(size);
        memcpy(&coef_num, data + off, sizeof(coef_num));
        off += sizeof(coef_num);
        const size_t data_bytes = static_cast<size_t>(size) * coef_num * sizeof(int16_t);
        if (size <= 0 || coef_num <= 0 || off + data_bytes > len) {
            return nullptr;
        }
        MfccFeature* mfcc = new MfccFeature(size, coef_num);
        if (!mfcc || !mfcc->feature_) {
            delete mfcc;
            return nullptr;
        }
        memcpy(mfcc->feature_, data + off, data_bytes);
        return mfcc;
    }
