...
public:
    ...
    {
        if (stackchan::custom::IsAddonUiModalActive()) {
            _event_shake = false;
            if (_is_reacting) {
        ...
        if (_is_reacting) {
            ...
            // Lock motion and move home once per reaction (skip during conversation).
            auto& motion = stackchan.motion();
            if (!stackchan::custom::IsConversationActive() && !stackchan::custom::ShouldHoldMotion() &&
                !_we_set_motion_lock && !motion.isModifyLocked()) {
                motion.setModifyLock(true);
                _we_set_motion_lock = true;
                motion.goHome(180);
            }
...
private:
    ...
    void restore_state(Modifiable& stackchan)
    {
        ...
        auto& motion = stackchan.motion();
        if (_we_set_motion_lock) {
            motion.setModifyLock(false);
            _we_set_motion_lock = false;
        }
    ...
    bool _we_set_motion_lock   = false;
