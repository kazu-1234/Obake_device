...
public:
    ...
    void _update(Modifiable& stackchan) override
    {
        ...
        const float freq_mult = stackchan::custom::GetIdleMotionFreqMultiplier();
        if (freq_mult > 0.01f) {
            delay = static_cast<uint32_t>(static_cast<float>(delay) / freq_mult);
            if (delay < 300) {
                delay = 300;
            }
        }
...
private:
    void perform_idle_motion(Modifiable& stackchan)
    {
        ...
        if (stackchan::custom::ShouldHoldMotion() || motion.isModifyLocked()) {
            return;
        }
        ...
        if (action < 50) {
            ...
            // 【动作 3：快速撇一眼】幅度と速度を抑えて急な首振りを防ぐ
            int target_yaw   = Random::getInstance().getInt(-320, 320);
            int target_pitch = Random::getInstance().getInt(100, 350);
            int speed        = Random::getInstance().getInt(140, 220);
