...
def clone_or_update_repo(
    ...
    if patch_path:
        apply_patch_if_clean(path, patch_path)


def apply_patch_if_clean(path, patch_path):
    patch_full_path = (
        patch_path if os.path.isabs(patch_path) else os.path.join(os.getcwd(), patch_path)
    )
...
def fetch_dependencies():
    ...
    for repo in repos:
        ...
        for extra in repo.get("extra_patches", []):
            extra_path = extra if os.path.isabs(extra) else os.path.join(script_dir, extra)
            apply_patch_if_clean(repo_path, extra_path)
